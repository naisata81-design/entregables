...
const ProjectSchema = new mongoose.Schema({
    ...
    estado: { type: String, enum: ['Borrador', 'Enviada', 'Aceptada', 'Rechazada', 'Activo', 'Pausado', 'Finalizado', 'Cotización', 'Aprobado', 'Terminado', 'Pagado', 'Levantamiento', 'Cotizando', 'En Seguimiento'], default: 'Levantamiento' },
    residenteId: String,
    residenteRespaldoId: String,
    ubicacion: String,
    assignedVehicles: [{ type: String, ref: 'Vehicle' }],
    assignedTools: [{ type: String, ref: 'InventoryItem' }],
    assignedPersonnel: [{ type: String }],
    documents: [{
        nombre: String,
        url: String,
        fecha: { type: Date, default: Date.now }
    ...
    finanzas: [{
        tipo: { type: String, enum: ['Ingreso', 'Egreso', 'Factura'], default: 'Factura' },
        folio: String,
        monto: Number,
        descripcion: String,
        urlArchivo: String,
    ...
    planificacionDiaria: [{
        ...
        encargadoDelDiaId: String,
        personalAsignado: [String],
        vehiculosAsignados: [{ type: String, ref: 'Vehicle' }],
        tareasDelDia: String,
        estado: { type: String, enum: ['Activa', 'Terminada', 'Cancelada'], default: 'Activa' }
    }]
}, { timestamps: true });
...
const QuoteSchema = new mongoose.Schema({
    _id: { type: String, default: () => new mongoose.Types.ObjectId().toString() },
    numeroCotizacion: { type: String, unique: true }, // Ej: C496
    fecha: { type: Date, default: Date.now },
    clienteId: { type: String, ref: 'Site' },
    elaboradaPorId: { type: String, ref: 'User' },
    ...
    iva: Number,
    total: Number,
    estado: { type: String, enum: ['Pendiente', 'Aprobada', 'Pagada', 'Cerrada'], default: 'Pendiente' }
}, { timestamps: true });
...
const InventoryTransactionSchema = new mongoose.Schema({
    _id: { type: String, default: () => new mongoose.Types.ObjectId().toString() },
    itemId: { type: String, ref: 'InventoryItem', required: true },
...
async function calcularVacacionesDinamicamente(user) {
    ...
    // Prioridad Absoluta: Si la DB tiene días cargados manualmente, usamos eso como base
    ...
    const ingreso = user.fechaIngreso ? new Date(user.fechaIngreso) : new Date(new Date().getFullYear(), 0, 1);
    const hoy = new Date();
    
    // Calcular meses exactos de antigüedad
    ...
    if (exactDiffMeses < 6) return 0; // Menos de 6 meses, sin derecho

    // Nueva Tabla de Ley Federal del Trabajo (LFT 2023)
    const getLFTDays = (yearIndex) => {
        if (yearIndex === 1) return 12;
        if (yearIndex === 2) return 14;
        if (yearIndex === 3) return 16;
        if (yearIndex === 4) return 18;
        if (yearIndex === 5) return 20;
        if (yearIndex >= 6 && yearIndex <= 10) return 22;
        if (yearIndex >= 11 && yearIndex <= 15) return 24;
        if (yearIndex >= 16 && yearIndex <= 20) return 26;
        if (yearIndex >= 21 && yearIndex <= 25) return 28;
        if (yearIndex >= 26 && yearIndex <= 30) return 30;
        return 32; // Fallback
    };

    // Historial de Derechos (Grants) según regla Naisata (Bolsas por año)
    let grants = [];
    
    // Evaluamos cada año posible (desde el 1 hasta el infinito, o más bien hasta los años que tenga + 1 para los adelantos)
    const currentYearEvaluating = Math.floor(exactDiffMeses / 12) + 1; // Si lleva 6 meses, está en el Año 1. Si lleva 18 meses, está en Año 2.

    for (let i = 1; i <= currentYearEvaluating; i++) {
        const totalDays = getLFTDays(i);
        const halfDays = totalDays / 2;
        
        // Fecha de caducidad para ambas mitades de este año: Exactamente al terminar el SIGUIENTE año
        // Ej: La bolsa del Año 1 caduca al cumplir el Año 2.
        let expDate = new Date(ingreso);
        expDate.setFullYear(expDate.getFullYear() + i + 1);

        // Mitad 1 (A los 6 meses de este ciclo)
        const monthForFirstHalf = (i - 1) * 12 + 6;
        if (exactDiffMeses >= monthForFirstHalf) {
            let d1 = new Date(ingreso);
            d1.setMonth(d1.getMonth() + monthForFirstHalf);
            grants.push({ remaining: halfDays, date: d1, expires: expDate });
        }

        // Mitad 2 (Al completarse el año)
        const monthForSecondHalf = i * 12;
        if (exactDiffMeses >= monthForSecondHalf) {
            let d2 = new Date(ingreso);
            d2.setMonth(d2.getMonth() + monthForSecondHalf);
            grants.push({ remaining: halfDays, date: d2, expires: expDate });
        }
    }

    // Historial de Consumos (Vacaciones tomadas)
    const consumos = await VacationRequest.find({
        userId: user._id.toString(),
        estado: 'aprobada'
    }).sort({ fechaInicio: 1 }); // Orden cronológico más antiguo a más reciente

    // Descontar usando sistema FIFO (Primeras entradas, primeras salidas)
    for (let req of consumos) {
        let daysToDeduct = req.diasSolicitados;
        let reqDate = new Date(req.fechaInicio);
        
        for (let grant of grants) {
            // Un grant es válido para descontar si caduca DESPUÉS de la fecha de la vacación solicitada
            // Quitamos la restricción de que se haya otorgado antes, para permitir descontar "adelantos" tomados antes de los 6 meses
            if (grant.expires > reqDate && grant.remaining > 0) {
                if (grant.remaining >= daysToDeduct) {
                    grant.remaining -= daysToDeduct;
                    daysToDeduct = 0;
                    break;
                } else {
                    daysToDeduct -= grant.remaining;
                    grant.remaining = 0;
                }
            }
        }
    ...
    // Calcular el balance válido HOY (suma de lo que queda de bolsas que no han caducado)
    let currentBalance = 0;
    for (let grant of grants) {
        if (grant.expires > hoy && grant.remaining > 0) {
            currentBalance += grant.remaining;
        }
    }
...
async function persistNotification(userId, payload) {
    try {
        ...
        // --- ENVIAR POR WHATSAPP MEDIANTE EL CRM ---
        const crmUrl = process.env.CRM_API_URL || 'https://crm-production-2af7.up.railway.app';
        let numeros = [];
        
        if (userId === 'all') {
            const users = await User.find({ telefono: { $exists: true, $ne: '' } });
            numeros = users.map(u => u.telefono);
        } else if (userId === 'admins') {
            const admins = await User.find({ rol: 'admin', telefono: { $exists: true, $ne: '' } });
            numeros = admins.map(u => u.telefono);
        } else {
            const user = await User.findById(userId);
            if (user && user.telefono) numeros.push(user.telefono);
        }
...
app.get('/api/users/:id/dashboard-stats', async (req, res) => {
    try {
        ...
        const vehiculosActualesRaw = await Vehicle.find({ currentUserId: user._id.toString(), estado: 'Prestado' });
        const vehiculosActuales = await Promise.all(vehiculosActualesRaw.map(async v => {
            ...
            return {
                ...
                modelo: v.modelo,
                placas: v.placas,
                fechaAceptacion: prestamoTx ? prestamoTx.fechaAceptacion : null,
                fechaSolicitudDevolucion: prestamoTx ? prestamoTx.fechaSolicitudDevolucion : null
            };
        }));
        ...
        const vehicles = await Vehicle.aggregate([
            ...
            },
            {
                $addFields: {
                    ...
                    currentStopStartTime: "$currentStopData.startTime"
                }
            },
            {
                $project: {
...
app.post('/api/vehicles', async (req, res) => {
    try {
        const { marca, modelo, color, placas, bitacoraEsperada, equipmentPhotos, documentosVehiculo, imei, gpsModel } = req.body;
        if (!marca || !modelo || !placas) return res.status(400).json({ error: 'Marca, modelo y placas son obligatorios.' });

        const newVehicle = new Vehicle({ marca, modelo, color, placas, bitacoraEsperada, equipmentPhotos, documentosVehiculo, imei, gpsModel });
...
app.post('/api/vehicles/:id/ghost', async (req, res) => {
    try {
        const v = await Vehicle.findById(req.params.id);
        if (!v) return res.status(404).json({error: 'No encontrado'});
...
app.post('/api/vehicles/:id/loan', async (req, res) => {
    try {
        ...
        vehicle.destinoSugeridoCRM = ''; // Limpiamos la etiqueta del CRM
        await vehicle.save();
        ...
        // Si se ligó a un proyecto, asegurar que aparezca en el dashboard del proyecto
        if (proyectoId) {
            try {
                const proj = await ProjectModel.findById(proyectoId);
                if (proj) {
                    if (!proj.assignedVehicles) proj.assignedVehicles = [];
                    if (!proj.assignedVehicles.some(v => v.toString() === vehicle._id.toString())) {
                        proj.assignedVehicles.push(vehicle._id);
                        await proj.save();
                    }
                }
            } catch(err) { console.error('Error linkeando vehiculo a proyecto', err); }
        }
        await tx.save();
        ...
        if (userId !== 'EXTERNO') {
            ...
            // NOTIFICAR VÍA WHATSAPP A TRAVÉS DEL BOT CRM (Interactivo)
            try {
                const crmUrl = process.env.CRM_API_URL || 'https://crm-production-2af7.up.railway.app';
                ...
                // Obtener la foto del equipo del vehículo para enviarla en WhatsApp
                const vehicleFullData = await Vehicle.findById(vehicle._id).select('equipmentPhotos marca').lean();
                const equipmentPhotoUrl = vehicleFullData?.equipmentPhotos?.[0] || null;

                await fetch(`${crmUrl}/api/whatsapp/asignacion-vehiculo`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        ...
                        placas: vehicle.placas || 'S/P',
                        checklist: parsedChecklist,
                        bitacoraRevisada: bitacoraRevisada || [],
                        checklistNotas: checklistNotas || '',
                        imgReporteDanos: imgReporteDanos ? `${DOMAIN}${imgReporteDanos}` : null,
                        equipmentPhotoUrl: equipmentPhotoUrl || null,
                        mainApiUrl: process.env.URL || 'https://naisa.newbox.mx'
                    })
                }).catch(e => console.error("Error silencioso fetch CRM", e));
            } catch (err) { console.error("Error trigger wa asignacion vehiculo", err); }
        }
...
app.post('/api/vehicles/:id/return', async (req, res) => {
    try {
        ...
        // Find the original Préstamo and set fechaDevolucion
        const prestamoTx = await VehicleTransaction.findOne({
            vehicleId: vehicle._id,
            userId: userId,
            tipoMovimiento: 'Préstamo'
...
app.post('/api/vehicles/:id/request-return', async (req, res) => {
    try {
        const vehicle = await Vehicle.findById(req.params.id);
        if (!vehicle) return res.status(404).json({ error: 'Vehículo no encontrado.' });
...
app.get('/api/users/:id/vehicles', async (req, res) => {
    try {
        // fecha almacenada como String ISO en MongoDB
...
app.get('/api/vehicles/:id/history-route', async (req, res) => {
    try {
        ...
        if (req.query.date) {
            // Ajustar para UTC-6 (Hora de Ciudad de México)
            const startOfDay = new Date(req.query.date + 'T06:00:00.000Z');
            
            // endOfDay será el inicio del siguiente día a las 05:59:59.999Z
...
app.get('/api/vehicles/:id/history-stops', async (req, res) => {
    try {
        ...
        const stops = await VehicleStop.find(query).sort({ startTime: -1 }); // newer first
...
app.get('/api/vehicles/:id/history', async (req, res) => {
    try {
        // fecha almacenada como String ISO en MongoDB, comparar como string
        const oneYearAgo = new Date();
        oneYearAgo.setFullYear(oneYearAgo.getFullYear() - 1);
        const oneYearAgoISO = oneYearAgo.toISOString();
        ...
        // Populate manual: vehicleId es String, Mongoose populate no funciona con String refs
...
app.get('/api/vehicle-transactions/all', async (req, res) => {
    try {
        ...
        // Populate manual de vehículos (Mongoose populate no funciona con String refs)
        const vehicleIds = [...new Set(txs.map(t => t.vehicleId))].filter(Boolean);
...
app.get('/api/employee-of-the-week', async (req, res) => {
    try {
        ...
        // Pre-fetch users
        const users = await User.find({ rol: { $in: ['user', 'Clase C'] } }).select('_id nombre apellido rol fechaIngreso fechaReinicioAsistencia');
        const userMap = {};
        ...
        for (const uid in userStats) {
            ...
            // En un sistema sin retardos, gana el que tenga más checkins (más constancia)
...
app.post('/api/vacations', async (req, res) => {
    try {
        ...
        // --- VALIDAR FECHAS EMPALMADAS ---
        const reqStart = new Date(fechaInicio);
        const reqEnd = new Date(fechaFin);

        const overlappingRequests = await VacationRequest.find({
            userId: userId,
            estado: { $in: ['pendiente', 'aprobada'] },
            fechaInicio: { $lte: reqEnd },
            fechaFin: { $gte: reqStart }
        });

        if (overlappingRequests.length > 0) {
            return res.status(400).json({ error: 'Ya tienes una solicitud (pendiente o aprobada) que choca con estos mismos días. Por favor revisa tu historial.' });
        }
        // ---------------------------------
...
app.put('/api/vacations/:id/status', async (req, res) => {
    try {
        ...
        if (estado === 'aprobada') {
            ...
            const diasDisponiblesCalculados = await calcularVacacionesDinamicamente(user);
            if (diasDisponiblesCalculados < request.diasSolicitados) {
                return res.status(400).json({ error: 'El usuario ya no tiene suficientes días para aprobar la solicitud.' });
            }
            // No restamos manualmente los días en la DB, ya que el estado 'aprobada'
            // automáticamente restará los días en el cálculo dinámico.
        }
...
app.put('/api/vacations/:id/revert', async (req, res) => {
    try {
        ...
        if (request.estado === 'aprobada') {
            // El reembolso de días ya no requiere suma estática, pues al cambiar el estado de 'aprobada'
            // a 'pendiente', la función dinámica automáticamente devolverá el saldo disponible.
        }
...
app.get('/api/quotes', async (req, res) => {
    try {
        const quotes = await QuoteModel.find()
            .populate('clienteId', 'nombre ubicacion')
            .populate('elaboradaPorId', 'nombre apellido')
            .sort({ createdAt: -1 });
        const mapped = quotes.map(q => ({ ...q.toObject(), id: q._id.toString() }));
        res.json(mapped);
    } catch (e) {
...
app.get('/api/quotes/next-number', async (req, res) => {
    try {
        ...
        if (lastQuote && lastQuote.numeroCotizacion) {
            const match = lastQuote.numeroCotizacion.match(/C(\d+)/);
            if (match) {
                nextNum = 'C' + (parseInt(match[1]) + 1);
            }
        }
        res.json({ nextNum });
    } catch (e) {
        res.status(500).json({ error: 'Error calculando consecutivo' });
    }
});
...
app.post('/api/quotes', async (req, res) => {
    try {
        ...
        await newQuote.save();
        const populated = await QuoteModel.findById(newQuote._id)
            .populate('clienteId', 'nombre ubicacion')
            .populate('elaboradaPorId', 'nombre apellido');
        const responseObj = { ...populated.toObject(), id: populated._id.toString() };
        res.status(201).json(responseObj);
    } catch (e) {
...
app.get('/api/settings/quotes-logo', async (req, res) => {
    try {
        let settings = await Settings.findOne({ tipo: 'quote_config' });
        res.json({ logo: settings ? settings.quoteLogo : null });
    } catch (e) {
        res.status(500).json({ error: 'Error' });
    }
});

app.put('/api/settings/quotes-logo', upload.single('logo'), async (req, res) => {
    try {
        if (!req.file) return res.status(400).json({ error: 'No image' });
        const logoData = `data:${req.file.mimetype};base64,${req.file.buffer.toString('base64')}`;
        let settings = await Settings.findOne({ tipo: 'quote_config' });
        if (!settings) settings = new Settings({ tipo: 'quote_config' });
        settings.quoteLogo = logoData;
        await settings.save();
        res.json({ message: 'Logo actualizado', logo: logoData });
    } catch (e) {
        console.error(e);
        res.status(500).json({ error: 'Error' });
    }
...
app.post('/api/projects', async (req, res) => {
    try {
        const { tipo, nombre, descripcion, numeroCotizacion, clienteId, presupuestoMateriales, presupuestoEstimado, estado, residenteId, ubicacion, assignedVehicles, assignedTools, assignedPersonnel, documents } = req.body;
...
// --- Nuevos Endpoints Gestión de Proyectos ---

app.post('/api/projects/:id/assign-vehicle', async (req, res) => {
    try {
        const { id } = req.params;
        const { vehicleId } = req.body;
        const project = await ProjectModel.findById(id);
        if(!project) return res.status(404).json({error: 'Proyecto no encontrado'});
        if(!project.assignedVehicles) project.assignedVehicles = [];
        if(!project.assignedVehicles.some(v => v.toString() === vehicleId)){
            project.assignedVehicles.push(vehicleId);
            await project.save();
        }
        res.json({ message: 'Vehículo asignado', project });
    } catch(e) {
        res.status(500).json({ error: 'Error asignando vehículo' });
    }
});

app.post('/api/projects/:id/assign-tool', async (req, res) => {
    try {
        const { id } = req.params;
        const { toolId } = req.body;
        const project = await ProjectModel.findById(id);
        if(!project) return res.status(404).json({error: 'Proyecto no encontrado'});
        if(!project.assignedTools) project.assignedTools = [];
        if(!project.assignedTools.some(t => t.toString() === toolId)){
            project.assignedTools.push(toolId);
            await project.save();
        }
        res.json({ message: 'Herramienta asignada', project });
    } catch(e) {
        res.status(500).json({ error: 'Error asignando herramienta' });
    }
});

app.post('/api/projects/:id/assign-personnel', async (req, res) => {
    try {
        const { id } = req.params;
        const { userId } = req.body;
        const project = await ProjectModel.findById(id);
        if(!project) return res.status(404).json({error: 'Proyecto no encontrado'});
        if(!project.assignedPersonnel) project.assignedPersonnel = [];
        if(!project.assignedPersonnel.includes(userId)){
            project.assignedPersonnel.push(userId);
            await project.save();
        }
        res.json({ message: 'Personal asignado', project });
    } catch(e) {
        res.status(500).json({ error: 'Error asignando personal' });
    }
});

app.post('/api/projects/:id/upload-document', upload.single('document'), async (req, res) => {
    try {
        const { id } = req.params;
        if (!req.file) return res.status(400).json({ error: 'No se subió archivo' });
        const urlBase64 = `data:${req.file.mimetype};base64,${req.file.buffer.toString('base64')}`;
        ...
        await project.save();
        res.json({ message: 'Documento subido', project });
    } catch(e) {
        res.status(500).json({ error: 'Error subiendo documento' });
    }
});

app.post('/api/projects/:id/finanzas', upload.single('archivo'), async (req, res) => {
    try {
        const { id } = req.params;
        const { tipo, folio, monto, descripcion, registradoPor } = req.body;
        
        let urlArchivo = '';
        let nombreArchivo = '';
        
        if (req.file) {
            urlArchivo = `data:${req.file.mimetype};base64,${req.file.buffer.toString('base64')}`;
        ...
        if(!project.finanzas) project.finanzas = [];
        
        project.finanzas.push({
            tipo: tipo || 'Factura',
            folio: folio || '',
            ...
            nombreArchivo,
            fecha: new Date(),
            registradoPor: registradoPor || 'Admin'
        });
        
        await project.save();
        res.json({ message: 'Registro financiero guardado', project });
    } catch(e) {
        console.error("Error saving finance record:", e);
        res.status(500).json({ error: 'Error guardando registro financiero' });
    }
});

// Endpoint para subir un avance diario
app.post('/api/projects/:id/avances', upload.single('evidencia'), async (req, res) => {
    try {
        const { id } = req.params;
        const { descripcion, registradoPor } = req.body;
        
        let urlEvidencia = '';
        let nombreEvidencia = '';
        
        if (req.file) {
            urlEvidencia = `data:${req.file.mimetype};base64,${req.file.buffer.toString('base64')}`;
            nombreEvidencia = req.file.originalname;
        }
        
        const project = await ProjectModel.findById(id);
        if(!project) return res.status(404).json({error: 'Proyecto no encontrado'});
        
        if(!project.avancesDiarios) project.avancesDiarios = [];
        
        project.avancesDiarios.push({
            descripcion: descripcion || '',
            urlEvidencia,
            nombreEvidencia,
            fecha: new Date(),
            registradoPor: registradoPor || 'Residente'
        });
        
        await project.save();
        res.json({ message: 'Avance registrado correctamente', project });
    } catch(e) {
        console.error("Error saving daily progress:", e);
        res.status(500).json({ error: 'Error guardando el avance diario' });
    }
});

// Endpoint para añadir una actividad
app.post('/api/projects/:id/actividades', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        await project.save();
        res.json({ message: 'Actividad añadida', project });
    } catch(e) {
        console.error("Error adding activity:", e);
        res.status(500).json({ error: 'Error añadiendo actividad' });
    }
});

// Endpoint para alternar estado de actividad
app.put('/api/projects/:id/actividades/:actividadId/toggle', async (req, res) => {
    try {
        ...
        const actividad = project.actividades.id(actividadId);
        if(!actividad) return res.status(404).json({error: 'Actividad no encontrada'});
        
        actividad.estado = actividad.estado === 'Pendiente' ? 'Completada' : 'Pendiente';
        
        await project.save();
        res.json({ message: 'Estado de actividad actualizado', project });
    } catch(e) {
...
// Endpoint para guardar/actualizar planificación diaria
app.post('/api/projects/:id/planificacion', async (req, res) => {
    try {
        const { id } = req.params;
        const { fechaAsignacion, horaInicio, horaFin, encargadoDelDiaId, personalAsignado, vehiculosAsignados, tareasDelDia } = req.body;

        const project = await ProjectModel.findById(id);
        if (!project) return res.status(404).json({ error: 'Proyecto no encontrado' });

        if (!project.planificacionDiaria) project.planificacionDiaria = [];

        const existingPlanIndex = project.planificacionDiaria.findIndex(p => p.fechaAsignacion === fechaAsignacion);

        if (existingPlanIndex >= 0) {
            project.planificacionDiaria[existingPlanIndex].horaInicio = horaInicio || "08:00";
            project.planificacionDiaria[existingPlanIndex].horaFin = horaFin || "18:00";
            project.planificacionDiaria[existingPlanIndex].encargadoDelDiaId = encargadoDelDiaId;
            project.planificacionDiaria[existingPlanIndex].personalAsignado = personalAsignado;
            project.planificacionDiaria[existingPlanIndex].vehiculosAsignados = vehiculosAsignados;
            project.planificacionDiaria[existingPlanIndex].tareasDelDia = tareasDelDia;
        } else {
            project.planificacionDiaria.push({
                fechaAsignacion,
        ...
        const responseObj = { ...project.toObject(), id: project._id.toString() };
        io.emit('updated_project', responseObj);
        
        res.json({ message: 'Planificación guardada', project: responseObj });
    } catch (e) {
        console.error("Error saving daily plan:", e);
        res.status(500).json({ error: 'Error guardando planificación' });
    }
});

app.put('/api/projects/:id/planificacion/:fecha/status', async (req, res) => {
    try {
        const { id, fecha } = req.params;
        const { estado } = req.body; // e.g. 'Terminada', 'Cancelada', 'Activa'
        
        const project = await ProjectModel.findById(id);
        if (!project) return res.status(404).json({ error: 'Proyecto no encontrado' });

        if (project.planificacionDiaria) {
            const plan = project.planificacionDiaria.find(p => p.fechaAsignacion === fecha);
            if (plan) {
                plan.estado = estado || 'Terminada';
                await project.save();
            }
        }
        ...
        res.json({ message: `Planificación marcada como ${estado || 'Terminada'}`, project: responseObj });
    } catch (e) {
        console.error("Error updating daily plan status:", e);
        res.status(500).json({ error: 'Error actualizando planificación' });
    }
});
...
app.get('/api/availability', async (req, res) => {
    try {
        const { fecha, currentProjectId } = req.query;
        ...
        const vacations = await VacationRequest.find({ estado: 'aprobado' });
        const targetDate = new Date(`${fecha}T12:00:00Z`); // Usa el medio día para evitar saltos
        vacations.forEach(v => {
            if (v.fechaInicio && v.fechaFin) {
                const start = new Date(`${v.fechaInicio}T00:00:00Z`);
                const end = new Date(`${v.fechaFin}T23:59:59Z`);
                if (targetDate >= start && targetDate <= end) {
                    conflicts.vacations.push(v.userId);
                }
            }
        });

        // 2. Proyectos Activos (Planificación Diaria)
        const projects = await ProjectModel.find({ estado: { $ne: 'Eliminado' } });
        projects.forEach(p => {
            if (String(p._id) === currentProjectId) return; // Ignore current project (allow moving around within same project)
            const plan = (p.planificacionDiaria || []).find(pl => pl.fechaAsignacion === fecha && pl.estado !== 'Terminada' && pl.estado !== 'Cancelada');
            if (plan) {
                const hInicio = plan.horaInicio || "08:00";
                const hFin = plan.horaFin || "18:00";
                const conflictData = { projectName: p.nombre, horaInicio: hInicio, horaFin: hFin };

                // Personal
                if (plan.encargadoDelDiaId) {
                    if (!conflicts.busyPersonnel[plan.encargadoDelDiaId]) conflicts.busyPersonnel[plan.encargadoDelDiaId] = [];
                    conflicts.busyPersonnel[plan.encargadoDelDiaId].push(conflictData);
                }
                (plan.personalAsignado || []).forEach(uid => {
                    if (!conflicts.busyPersonnel[uid]) conflicts.busyPersonnel[uid] = [];
                    conflicts.busyPersonnel[uid].push(conflictData);
                });

                // Vehículos
                (plan.vehiculosAsignados || []).forEach(vid => {
                    if (!conflicts.busyVehicles[vid]) conflicts.busyVehicles[vid] = [];
                    conflicts.busyVehicles[vid].push(conflictData);
                });
            }
        });

        // 3. Vehículos en Préstamo Activo
        const activeLoans = await LoanModel.find({ status: 'activo', isVehicle: true });
        activeLoans.forEach(loan => {
            const vid = String(loan.vehicleId);
            if (!conflicts.busyVehicles[vid]) conflicts.busyVehicles[vid] = [];
            conflicts.busyVehicles[vid].push({ projectName: 'Préstamo Activo', type: 'loan', horaInicio: "00:00", horaFin: "23:59" });
        });

        res.json(conflicts);
    } catch(e) {
        console.error("Error en motor de disponibilidad:", e);
...
app.post('/api/inventory/transaction', async (req, res) => {
    try {
        ...
        for (let i = 0; i < txItemsArr.length; i++) {
            ...
            // Automáticamente vincular herramienta al proyecto si aplica
            if (proyectoId && proyectoId !== 'OTRO' && !isInsumo && tipoMovimiento === 'Salida') {
                try {
                    const ProjectModel = mongoose.model('Project');
                    const project = await ProjectModel.findById(proyectoId);
                    if (project) {
                        if (!project.assignedTools) project.assignedTools = [];
                        const toolIdStr = itemTx.itemId.toString();
                        if (!project.assignedTools.includes(toolIdStr)) {
                            project.assignedTools.push(toolIdStr);
                            await project.save();
                        }
                    }
                } catch(err) {
                    console.error("Error linking tool to project:", err);
                }
            }
            
            // Si es Devolución, opcionalmente podríamos removerlo, pero lo dejaremos en el historial del proyecto por ahora.
        }
...
async function runCronRoutine() {
    try {
        ...
        if (isHoliday(todayStr) && !sentHolidayNotifications[`today_${todayStr}`]) {
            const title = "¡Día de Descanso Oficial!";
            const body = `Hoy es un día de descanso obligatorio por ley. ¡Disfruta tu día!`;
        ...
        // Verificar si MAÑANA es feriado y aún no hemos notificado
        if (isHoliday(tomorrowStr) && !sentHolidayNotifications[`prep_${tomorrowStr}`]) {
            const title = "¡Próximo Descanso Oficial!";
            const body = `Te recordamos que mañana (${tomorrowStr}) es día de descanso obligatorio por ley.`;

            await notifyAll({ title, body }).catch(e => console.error("Error push:", e));
            sentHolidayNotifications[`prep_${tomorrowStr}`] = true;
            console.log(`[Cron] Notificación preventiva enviada para el feriado (mañana): ${tomorrowStr}`);
        }
    } catch (e) {
...
app.get('/api/it/trigger-holidays', async (req, res) => {
    await runCronRoutine();
    res.json({ message: 'Holiday check triggered.' });
});

app.get('/api/it/fix-jac', async (req, res) => {
    try {
        const jac = await Vehicle.findOne({ $or: [{ marca: /jac/i }, { modelo: /jac/i }] });
        if (jac && jac.currentStopId) {
            const stop = await VehicleStop.findById(jac.currentStopId);
            if (stop) {
                stop.endTime = new Date();
                stop.durationMinutes = Math.max(0, Math.round((new Date() - stop.startTime) / 60000));
                await stop.save();
            }
            jac.currentStopId = null;
            await jac.save();
            res.json({ message: `Stop cleared successfully for ${jac.marca} ${jac.modelo}.` });
        } else if (!jac) {
            res.json({ message: 'Error: Vehicle JAC not found in database.' });
        } else {
            res.json({ message: `Vehicle ${jac.marca} found, but no stop to clear.` });
        }
    } catch(e) {
        res.status(500).json({error: e.message});
    }
...
// --- Flespi GPS Webhook ---
// Buffer en memoria para diagnóstico — guarda los últimos 10 payloads recibidos
const flespiDebugLog = [];
app.get('/api/flespi/debug', (req, res) => {
        ...
        if (Array.isArray(data)) {
            for (let msg of data) {
                ...
                if (imei && lat !== undefined && lng !== undefined) {
                    ...
                    if (vehicle) {
                        if (vehicle.ghostMode) {
                            // console.log(`[FLESPI WEBHOOK] Vehículo ${imei} en Ghost Mode. Ignorando ruta.`);
                            continue;
                        }
                        ...
                        const isMoving = speed > 3; // Ignorar drift GPSmenor a 3km/h
                        ...
                        if (vehicle.currentStopId) {
                            ...
                            if (activeStop) {
                                currentStopStartTime = activeStop.startTime;
                            }
                        }
